...
#	std_sum = np.sum(std_arr**2)
	std_sum = np.sum(std_arr**2,axis=1)
	error = np.sqrt(std_sum)
	return error
...
def exp_error(dd_df):
	return np.log(2)*np.exp2(-dd_df.iloc[0,:])*dd_df.iloc[1,:]
...
#		print std_arr
	...
	std_d_df.columns=mean_d_df.columns
...
#	dd_dict = OrderedDict()
	...
	for lig in lig_list:
		if lig != vehicle:
			temp_mean = mean_d_df[lig]-mean_d_df[vehicle]
			temp_std = dd_err(std_d_df[[vehicle,lig]])
			for row in range(0,mean_d_df.shape[0]):
				dd_df['{1}_{0}'.format(
					lig,mean_d_df.index[row])]=[temp_mean[row],temp_std[row]]
	'''
	for row in range(0,mean_d_df.shape[0]):
...
#	dd_df = pd.DataFrame.from_dict(dd_dict,orient='index')
	print dd_df
	exp_df = pd.DataFrame(columns=dd_df.columns)
	exp_df = exp_df.append(np.exp2(-dd_df.iloc[0,:]),ignore_index=True)
	exp_df = exp_df.append(exp_error(dd_df),ignore_index=True)
	'''
	for row in range(0,dd_df.shape[0]):
		exp_df[dd_df.index.values[row]] = np.array([np.exp2(-dd_df.iat[row,0]),
			exp_error(dd_df,row)])
	'''
	exp_df.index = ['exp2','std']
	print exp_df
	return dd_df,exp_df
